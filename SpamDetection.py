...
global ELM_acc,random_acc,nb_acc,svm_acc
...
def extremeMachineLearning():
    ...
    ELM_acc = cal_accuracy(y_test, prediction_data,'Extreme Machine Learning Algorithm Accuracy')
    precision = precision_score(y_test, prediction_data,average='macro') * 100
    recall = recall_score(y_test, prediction_data,average='macro') * 100
    fmeasure = f1_score(y_test, prediction_data,average='macro') * 100
    text.insert(END,"ELM Precision : "+str(precision)+"\n")
    text.insert(END,"ELM Recall : "+str(recall)+"\n")
    text.insert(END,"ELM FMeasure : "+str(fmeasure)+"\n")

def accuracyComparison():
    height = [random_acc,nb_acc,svm_acc,ELM_acc]
...
font = ('times', 16, 'bold')
title = Label(main, text='Unveiling The Power of Extreme Learning Machine: Combatting Spam and Identifying Fake Users On Twitter ')
...
exitButton = Button(main, text="Run ELM Algorithm", command=extremeMachineLearning)
